...
hold on
stem(X,'LineWidth',1)
stem(X_hat_s,'LineWidth',1)
stem(X_hat_r,'LineWidth',1)
hold off
title('Simulated Truing Solution')
legend('actual','predicted','regularized')
ylabel('Adjustment Error [revs]')
xlabel('Spoke Number')
ax = gca;
ax.FontSize=16;
...
figure()
subplot(4,1,1)
plot(theta_r,IM_lat*X_hat_s-YLat)
title('Lateral Error')
ylabel('Lateral [mm]')
ax = gca;
ax.FontSize=16;
subplot(4,1,2)
plot(theta_r,IM_rad*X_hat_s-YRad)
title('Radial Error')
ylabel('Radial [mm]')
ax = gca;
ax.FontSize=16;
subplot(4,1,3)
bar(theta_s,IM_ten*X_hat_s-Y_ten)
title('Tension Error')
ylabel('Tension [N]')
ax = gca;
ax.FontSize=16;
subplot(4,1,4)
stem(theta_s, X_hat_s - X)
title('Spoke Adjustment Error')
ylabel('Error [revs]')
xlabel('Angle [rad]')
ax = gca;
